"""MLX-specific adapters for DeepSeek MoE models."""
...
class DeepSeekMlxAdapter(BaseModelAdapter):
    """MLX adapter for DeepSeek MoE models."""

    def get_num_experts(self, model: Any, layer_idx: int) -> int:
        """Get number of experts in DeepSeek layer."""
    ...
    def get_experts_per_tok(self, model: Any, layer_idx: int) -> int:
        ...
        for attr in ['num_experts_per_tok', 'top_k', 'k']:
            if hasattr(moe_block, attr):
                return getattr(moe_block, attr)
    ...
    def get_expert_indices_from_output(self, output: Any) -> List[int]:
        ...
        if isinstance(output, tuple):
            for item in output:
                if hasattr(item, 'expert_mask'):
                    mask = item.expert_mask
                    if hasattr(mask, 'nonzero'):
                        indices.extend(mask.nonzero().tolist())
                elif isinstance(item, nn.Module) and hasattr(item, 'shape'):
                    # Top-k indices
                    k = 4  # Default for DeepSeek
                    if item.ndim >= 2:
                        top_k = item.argsort()[-k:]
                        indices.extend(top_k.flatten().tolist())
